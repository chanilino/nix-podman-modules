...
with lib;
let
  cfg = config.virtualisation.podman-rootless-containers;
  ...
  containerOptions =
    { ... }: {
      ...
      options = {
        ...
        hostUser = mkOption {
          type = with types; str;
          description = lib.mdDoc "Host user to run containers.";
          example = "username";
        };
  ...
  mkService = name: container: let
    ...
    run_dir_path = "/run/user/" + builtins.toString config.users.users.${container.hostUser}.uid;
    cid_path = "${run_dir_path}/podman-${escapedName}.ctr-id"; 
  in {
    ...
    preStop = ''
      [ -f ${cid_path} ] && ${cfg.backend} stop --ignore --cidfile=${cid_path}
      [ -f ${cid_path} ] || ${cfg.backend} stop ${name} 
       ${cfg.backend} rm -f --ignore --cidfile=${cid_path}
      '';

    serviceConfig = {
      ### There is no generalized way of supporting `reload` for docker
      ### containers. Some containers may respond well to SIGHUP sent to their
      ### init process, but it is not guaranteed; some apps have other rel
...
in {
  ...
  options.virtualisation.podman-rootless-containers = {
